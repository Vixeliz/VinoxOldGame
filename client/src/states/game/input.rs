...
pub fn move_player(
    ...
    if let Ok(mut player_velocity) = velocity_query.get_single_mut() {
        ...
        player_velocity.linvel.x = (right - left) * 250. * time.delta_seconds();
        
        let forward = if input.pressed(KeyCode::W) { 1. } else { 0. };
        let back = if input.pressed(KeyCode::S) { 1. } else { 0. };
        player_velocity.linvel.z = (back - forward) * 250. * time.delta_seconds();
        
        let up = if input.pressed(KeyCode::Space) { 1. } else { 0. };
        let down = if input.pressed(KeyCode::C) { 1. } else { 0. };
        player_velocity.linvel.y = (up - down) * 250. * time.delta_seconds();
    }
}
